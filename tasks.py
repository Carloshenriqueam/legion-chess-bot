...
async def process_challenge_result(bot, challenge):
    """Processa o resultado de um desafio específico (usado pelo botão manual)."""
    try:
        challenge_id = challenge['id']
        game_url = challenge.get('game_url')
        
        if not game_url:
            logger.error(f"Desafio {challenge_id} não tem game_url")
            return False, None
            
        logger.info(f"🔍 Processando desafio manual {challenge_id} com game_url {game_url}")
        
        # Obter resultado da partida
        outcome = await lichess_api.get_game_outcome(game_url)
        if not outcome or not outcome.get('finished'):
            logger.warning(f"Partida {game_url} ainda não terminou")
            return False, None
            
        # Determinar jogadores
        challenger_id = challenge['challenger_id']
        challenged_id = challenge['challenged_id']
        is_swiss_game = challenge.get('swiss_pairing_id') is not None
        
        if is_swiss_game:
            challenger_id = challenge.get('player1_id')
            challenged_id = challenge.get('player2_id')
        ...
        result = None
        rating_changes = None
        mode = challenge.get('time_control_mode', 'blitz')
        is_rated = bool(challenge.get('is_rated', False))
        is_draw = outcome.get('is_draw', False)
        ...
        if not linked_white and not linked_black:
            # Ambos anônimos
            await database.mark_challenge_as_finished(challenge_id, winner_id, loser_id, result, outcome.get('pgn') or game_url)
            await database.update_challenge_status(challenge_id, 'finished')
        elif linked_white and not linked_black:
            # Apenas white vinculado
            linked_player = p_white['discord_id']
            other_player = challenger_id if str(challenger_id) != str(linked_player) else challenged_id
            winner_id = linked_player
            loser_id = other_player
            ...
            if not is_swiss_game:
                if result == 'win':
                    ...
                    await database.update_player_stats(challenger_id, mode, 'draw')
                    await database.update_player_stats(challenged_id, mode, 'draw')
                    if is_rated:
                        rating_changes = await database.apply_draw_ratings(challenger_id, challenged_id, mode)
        
        logger.info(f"✅ Desafio {challenge_id} processado com sucesso")
        return True, rating_changes
        
    except Exception as e:
        logger.error(f"❌ Erro ao processar desafio {challenge.get('id', 'unknown')}: {e}", exc_info=True)
        return False, None
